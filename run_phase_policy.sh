...
# discharge pinned at 0 in the GROWTH phase only. gpei holds it at exactly 0 for the
# first ~100 h, and phase 0 is biomass growth -- the vessel should be filling.
FIX_P0=${FIX_P0:-0}
...
PIDS=()
for P in 0 1 2; do
  FIX=""; [ "$P" = "0" ] && [ -n "$FIX_P0" ] && FIX="-fix_discharge $FIX_P0"
  J=$(sbatch --parsable $SB --job-name="ph$P" --time=14:00:00 \
    ...
    --wrap="$CONDA python -u policy_learning/cdil_policy_phase.py \
            -phase_prefix $PREFIX -phase $P -lam $LAM -iters $ITERS $FIX \
...
sbatch --dependency=afterok:$DEP $SB --job-name=ph_expl --time=06:00:00 \
  ...
  --wrap="$CONDA python -u policy_learning/explore_with_policy.py \
          -phase_policies $RES/phase -fix_discharge $FIX_P0 -fix_until $PHASE0_END \
          -out $DATA -tag phase -n 1 -p_dropout 0.0"
