...
try:
    ...
    HYDROGYM_AVAILABLE = False
    warnings.warn('Hydrogym is not installed!')

    # `class HydroSVDWrapper(FlowEnv)` is evaluated at import time, so the
    # name must exist even when hydrogym is absent. This placeholder lets the
    # module import; instantiating a Hydro* env still raises, which is correct.
    class FlowEnv:
        def __init__(self, *args, **kwargs):
            raise ImportError(
                'Hydrogym is not installed -- Hydro* environments are '
                'unavailable. Install hydrogym to use them.'
            )
...
# Don't require that the user has the SMPL/PenSimPy stack installed
try:
    from sindy_rl.pensim_env import PenSimGymEnv, PenSimGymEnvWithTime
except ImportError:
    warnings.warn('PenSim (smpl/PenSimPy) is not installed!')
...
try:
    from latent_repr.latent_env import PenSimLatentEnv, PenSimLatentEnvWithTime
except ImportError:
    warnings.warn('latent_repr is not available!')
