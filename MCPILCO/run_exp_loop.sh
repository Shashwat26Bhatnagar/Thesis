...
#
# Everything is isolated: results_exp/ and configdata/pensim_exp/, prefix "exp".
#
...
if [ ! -f "$RES/exp_model_iter0_phase0.pt" ]; then
  for p in 0 1 2; do cp "${SRC_PREFIX}_phase${p}.pt" "$RES/exp_model_iter0_phase${p}.pt"; done
...
echo "   lambda=$LAM  policy iters=$ITERS  dropout=$DROPOUT  clip=BUGGY_RECIPE_CLIP"
echo "   data  : $DATA ($(ls "$DATA"/*.csv | wc -l) CSVs)"
echo "=========================================================================="

DEP=""
for IT in $(seq "$FROM" "$TO"); do
  MODEL=$RES/exp_model_iter${IT}
  NEXT=$RES/exp_model_iter$((IT+1))
  POL=$RES/exp_policy_iter${IT}.pt
  PREV=$RES/exp_policy_iter$((IT-1)).pt
  WARM=""; [ -f "$PREV" ] && WARM="-init_policy $PREV"
  D=""; [ -n "$DEP" ] && D="--dependency=afterok:$DEP"

  echo "--- iteration $IT"

  J1=$(sbatch --parsable $SB $D --job-name="ep${IT}" --time=14:00:00 \
    --output="$RES/it${IT}_policy_%j.out" --error="$RES/it${IT}_policy_%j.err" \
    --wrap="$CONDA python -u policy_learning/exp_policy.py \
            -phase_prefix $MODEL -lam $LAM -iters $ITERS -out $POL $WARM")
  ...
  J2=$(sbatch --parsable $SB --dependency=afterok:$J1 --job-name="ex${IT}" --time=06:00:00 \
    ...
    --wrap="$CONDA BUGGY_RECIPE_CLIP=1 python -u policy_learning/explore_with_policy.py \
            -policy $POL -cliprecipe -out $DATA -tag exp_iter${IT} \
    ...
    for P in 0 1 2; do
      J=$(sbatch --parsable $SB --dependency=afterok:$J2 --job-name="em${IT}p$P" \
        ...
        --wrap="$CONDA python -u train_rbf_pensim.py -phase $P -data_dir $DATA \
                -tag exp_it$((IT+1))_p${P} -save_dir $RES -n_keep $N_KEEP \
                -n_epoch $N_EPOCH -select pivchol -std_from ${SRC_PREFIX}_phase0.pt; \
                cp $RES/rbf_model_exp_it$((IT+1))_p${P}.pt ${NEXT}_phase${P}.pt")
      TIDS+=("$J")
    done
    DEP=$(IFS=:; echo "${TIDS[*]}")
...
echo "W2    : grep -h '^iter' $RES/it*_policy_*.out | tail -20"
echo "yields: grep -h 'total yield' $RES/it*_explore_*.out"
echo "rows  : head -3 $DATA/exp_iter0_batch_0.csv"
