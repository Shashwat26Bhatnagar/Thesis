...
SAVE_DIR = os.path.join(_REPO, "results_reps")
...
_ap.add_argument("-eta", type=float, default=None, help="W2 constraint threshold")
_ap.add_argument("-alpha_w2", type=float, default=None, help="Lagrange weight")
_ap.add_argument("-kappa", type=float, default=None, help="LCB coefficient")
_ap.add_argument("-lam", type=float, default=0.0, help="optional L2 on ||a||^2")
...
print(f"\nreward GP: {os.path.basename(_args.reward_model)}  "
      f"train pts={RMODEL.gp_inputs.shape[0]}  held-out R^2={_rck.get('held_out_r2')}")
print(f"  yield/step mu={R_MU:.4f} sd={R_SD:.4f}; the objective maximises the LOWER")
print(f"  confidence bound mu_r - {KAPPA}*sigma_r, not the mean")

print(f"\nREPS constraint: max reward  s.t.  W2_h <= eta")
print(f"  eta      = {ETA}   (measured: imitation-only policies converge to "
      f"0.219-0.253 per hour in training units)")
print(f"  alpha_W2 = {ALPHA_W2}   penalty = alpha * relu(W2_h - eta), ONE-SIDED")
...
def window_loss(t, s, a, mu, cov, s_next):
    ...
    m_list, v_list = RMODEL.get_gp_estimate(gp_inputs=r_in, gp_index_list=[0])
    r_lcb = (m_list[0].reshape(-1)
             - KAPPA * torch.sqrt(v_list[0].reshape(-1).clamp_min(1e-12)))
    reward = r_lcb.mean()
...
for it in range(N_ITERS):
    ...
    if l2_first is None:
        l2_first = l2m
    hist.append(w2m)
    w2a = np.array(_log["w2"])
    n_viol = int((w2a > ETA).sum())
    print(f"iter {it:3d}  loss={L.mean():.5f}   reward(LCB)={np.mean(_log['r']):.5f}"
          f"   W2 mean={w2m:.5f} min={w2a.min():.5f} max={w2a.max():.5f}", flush=True)
    print(f"          eta={ETA}  hours violating={n_viol}/{len(w2a)}"
          f"  mean excess={np.mean(_log['viol']):.5f}"
          f"  penalty={np.mean(_log['pen']):.5f}"
          f"  (penalty/|reward|={np.mean(_log['pen'])/max(abs(np.mean(_log['r'])),1e-9):.2f}x)",
          flush=True)
