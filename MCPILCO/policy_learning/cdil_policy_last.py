...
SAVE_DIR = os.path.join(_REPO, "results_last")
...
_ap.add_argument("-sum_steps", action="store_true",
                 help="sum all 5 step covariances (the old behaviour) instead of "
                      "taking the 5th step alone")
_ap.add_argument("-last_scale", type=float, default=1.0,
                 help="scale the 5th-step covariance; 5.0 compensates for the "
                      "expert's 1 h interval against the GP's 0.2 h")
...
LAMBDA_A = _args.lam
LAST_STEP_ONLY = not _args.sum_steps
LAST_SCALE = _args.last_scale
if _args.iters:
    N_ITERS = _args.iters
...
print(f"covariance compared: "
      + (f"5th step ONLY x{LAST_SCALE}  (BPTT still through all {STEPS_PER_EXPERT} "
         f"steps)" if LAST_STEP_ONLY
         else f"SUM over all {STEPS_PER_EXPERT} steps"))
