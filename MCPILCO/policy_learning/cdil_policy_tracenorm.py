...
# -*- coding: utf-8 -*-
"""
policy_learning/cdil_policy_tracenorm.py

CDIL policy optimization, rebuilt clean. Writes to results_clean/ and does not touch
any existing file.

WHY THIS FILE EXISTS: THE IMITATION TERM WAS SILENT
...
STRUCTURE (MC-PILCO, Amadio et al. 2022)
    ...
    python policy_learning/cdil_policy_tracenorm.py \\
...
SAVE_DIR = os.path.join(_REPO, "results_tracenorm")
...
if _args.iters:
    N_ITERS = _args.iters
OUT = _args.out or os.path.join(SAVE_DIR, f"tn_lam{str(LAMBDA_A).replace('.','p')}.pt")
