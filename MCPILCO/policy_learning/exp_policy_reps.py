...
# -*- coding: utf-8 -*-
"""
policy_learning/exp_policy_reps.py
...
THE OBJECTIVE
    ...
    loss = -LCB_reward  +  ALPHA_W2 * relu(W2_h - eta)  +  LAMBDA_A * mean||a||^2
                        +  chance penalties
...
STRUCTURE (MC-PILCO, Amadio et al. 2022)
    ...
    python policy_learning/exp_policy_reps.py \\
...
SAVE_DIR = os.path.join(_REPO, "results_expreps")
...
if _args.iters:
    N_ITERS = _args.iters
OUT = _args.out or os.path.join(SAVE_DIR, "exp_reps_policy.pt")
...
for it in range(N_ITERS):
    ...
    if l2_first is None:
        l2_first = l2m
    hist.append(w2m)
    w2a = np.array(_log["w2"]); rm = np.mean(_log["r"]); pm = np.mean(_log["pen"])
    ...
    print(f"          eta={ETA} violating={int((w2a>ETA).sum())}/{len(w2a)} "
          f"excess={np.mean(_log['viol']):.5f} penalty={pm:.5f} "
