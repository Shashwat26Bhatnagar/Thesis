...
class Model_learning_RVGP(Model_learning):
    """Model learning with ONE joint RVGP (Matern-on-manifold) GP over all state dims.

    Unlike stock MC-PILCO (num_gp = D independent scalar GPs), this uses a single GP
    (num_gp = 1) whose kernel is RVGP Eq. 15. The manifold is built ONCE from the
    training inputs; the GP input is the (N*D, k) positional encoding, the target is the
    stacked state-delta (N*D, 1). Cross-dimension coupling comes from the connection
    Laplacian. Encodings/eigenvalues are frozen geometry; training only fits the three
    Matern hyperparameters (nu, kappa, sigma_f) + sigma_n via marginal likelihood.

    Scope: model learning + one-step / open-loop prediction. The rollout re-encodes query
    points with (non-differentiable) Nystrom, so do NOT backprop the policy gradient
    through the state for this model (use it for model learning only, as agreed).
    """
    ...
    def get_gp(self, gp_index, init_dict):
        if self.rvgp is None:
            ...
            num_vertices = self.rvgp.num_vertices
            k = self.rvgp.k
        d = dict(init_dict)
        d.update(
            active_dims=np.arange(k),
    ...
    def add_data(self, new_state_samples, new_input_samples):
        ...
        if self._raw is None:
            ...
            self._raw = torch.cat([self._raw, raw], 0)
    ...
    def build_manifold(self):
        ...
        self.rvgp = RVGPManifold(
            X=self._raw.detach().cpu().numpy(),
            n_out_dims=self.dim_state,
            dtype=self.dtype,
            device=self.device,
            **self.rvgp_dict,
        )
    ...
    def get_next_state_from_gp_output(
        ...
        delta_mean = gp_output_mean_list[0].reshape(-1, D)
        delta_var = gp_output_var_list[0].reshape(-1, D)
        if particle_pred:
            delta_var = torch.clamp(delta_var, min=1e-12, max=1e6)
            delta_sample = Normal(delta_mean, torch.sqrt(delta_var)).rsample()
        else:
            delta_sample = delta_mean
        next_states = current_state + delta_sample
        return next_states, delta_mean, delta_var
...
class Model_learning_RVGP(Model_learning):
    """One joint RVGP (Matern-on-manifold) GP over the state, with periodic-angle handling.

    The pole angle (and any periodic state) is wrapped: theta and theta+2pi are the SAME
    physical state but far apart as raw numbers, which would tear the RVGP proximity graph
    at the +/-pi seam. Fix: the manifold input embeds each angle as (cos, sin), so the graph
    distances are geometrically correct.

    Because RVGP's vector field lives in the manifold's AMBIENT space, the angle's delta is
    predicted in that same (cos, sin) plane (Delta_cos, Delta_sin) and the raw angle is
    reconstructed with atan2 at the end. Non-angle states and their deltas are untouched;
    the returned next state is exact.

    Manifold ambient (per point):   e(x) = [x_notangle, cos(x_angle), sin(x_angle)] , then [e(x), u]
    GP target (per point):          Delta_e = e(x_{t+1}) - e(x_t)          (dim D_e)
    Reconstruction:                 notangle: x + Delta ; angle: atan2(sin+Dsin, cos+Dcos)

    Scope: model learning + one-step / open-loop prediction (rollout re-encodes via
    non-differentiable Nystrom, so do not backprop the policy gradient through the state).
    """
    ...
    def __init__(
        ...
        self._matern_init_dict = dict(init_dict)
        self.rvgp_dict = dict(rvgp_dict)
        self.angle_indices = list(angle_indices)
    ...
    def data_to_gp_input(self, states, inputs):
        ...
        return self.rvgp.encode(z)

    
    def data_to_gp_output(self, states):
        e = self._expand_state(states)
        delta_e = e[1:, :] - e[:-1, :]
        return [delta_e.reshape(-1, 1)]

    def data_to_gp_IO(self, states, inputs):
        e = self._expand_state(states)
        z = torch.cat([e, inputs], dim=1)[:-1, :]
        gp_in = self.rvgp.encode(z)
    ...
    def get_next_state_from_gp_output(
        ...
        De = self.dim_e
        n_na, n_a = len(self.not_angle_indices), len(self.angle_indices)
        delta_mean = gp_output_mean_list[0].reshape(-1, De)
        delta_var = gp_output_var_list[0].reshape(-1, De)
        if particle_pred:
            var_safe = torch.clamp(delta_var, min=1e-12, max=1e6)
            de = Normal(delta_mean, torch.sqrt(var_safe)).rsample()
        else:
            de = delta_mean
        ...
        cur_ang = current_state[:, self.angle_indices]
        next_cos = torch.cos(cur_ang) + de_cos
        next_sin = torch.sin(cur_ang) + de_sin
        next_states[:, self.angle_indices] = torch.atan2(next_sin, next_cos)
        return next_states, delta_mean, delta_var

    def add_transitions(self, obs, act, next_obs):
        """Explicit (s,a,s') triples -- safe for subsampled/shuffled data, unlike
        add_data() which assumes consecutive rows."""
        s  = torch.as_tensor(obs,      dtype=self.dtype, device=self.device)
        a  = torch.as_tensor(act,      dtype=self.dtype, device=self.device)
        s2 = torch.as_tensor(next_obs, dtype=self.dtype, device=self.device)
        self.dim_state, self.dim_input = s.shape[1], a.shape[1]
        e, e2 = self._expand_state(s), self._expand_state(s2)
        z, delta_e = torch.cat([e, a], dim=1), e2 - e
        if self._z is None:
            self._z, self._delta_e = z, delta_e
        else:
            self._z = torch.cat([self._z, z], 0)
    ...
    def predict_transition(self, obs, act, particle_pred=True):
        """Frozen-model sampler for Dyna: (s,a) -> s'."""
        s = torch.as_tensor(obs, dtype=self.dtype, device=self.device)
        a = torch.as_tensor(act, dtype=self.dtype, device=self.device)
        with torch.no_grad():
            gp_in = self.data_to_gp_input(s, a)
            mean_list, var_list = self.get_gp_estimate(gp_inputs=gp_in, gp_index_list=[0])
            return self.get_next_state_from_gp_output(
                current_state=s, current_input=a,
                gp_output_mean_list=mean_list, gp_output_var_list=var_list,
                particle_pred=particle_pred)
